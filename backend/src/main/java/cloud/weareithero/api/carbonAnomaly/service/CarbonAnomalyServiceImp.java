...
public class CarbonAnomalyServiceImp implements CarbonAnomalyService {
    ...
    public ResponseDTO getAnomalyList(CarbonAnomalyRequestDTO dto) {
        ...
        try {
            ...
            // 2. 계산된 날짜를 DTO에 담습니다. (연도 선택 안 하면 null이 들어감)
            ...
            // 3. DB 조회 (MyBatis에서 calculatedStartDate가 null이면 조건 무시됨)
            ...
            return ResponseDTO.builder().status(false).message("데이터 조회 중 오류가 발생했습니다.").build();
        }

        return ResponseDTO.builder().status(isSuccess).data(resultMap).message(message).build();
    }
