...
public interface AsnControllerDocs {
  ...
  public ResponseDTO findAll(@RequestBody(
    content = @Content(
      schema = @Schema(implementation = AsnRequestDTO.class),
      examples = {
        @ExampleObject(
            name = "1. 전체 검색 예시",
            value = AsnResponseExamples.SUCCESS1_DEFAULT,
            description = "전체 기간 동안의 입고 내역을 조회할 때 사용합니다."
        ),
        @ExampleObject(
            name = "2. 입고 ID 단건 검색 예시",
            value = AsnResponseExamples.SUCCESS2_DEFAULT,
            description = "날짜 상관없이 특정 입고 번호로만 검색할 때 사용합니다."
        ),
        @ExampleObject(
            name = "3. 날짜 범위 검색 예시",
            value = AsnResponseExamples.SUCCESS3_DEFAULT,
            description = "특정 기간 동안의 입고 내역을 조회할 때 사용합니다."
        ),
        @ExampleObject(
            name = "4. 날짜 범위 및 입고 ID 검색 예시",
            value = AsnResponseExamples.SUCCESS4_DEFAULT,
            description = "특정 기간 동안의 해당 입고 Id 내역을 조회할 때 사용합니다."
        ),
      }
    )) 
